"""
simpan_model_rf.py - Training ulang dengan RandomForest dan simpan model
"""
...
df_kom = pd.read_csv("data/KOMODITAS_GABUNGAN.csv", index_col=0)
...
with open("models/models_rf.pkl", "wb") as f:
    pickle.dump(models, f)

avg = sum(v["acc_test"] for v in models.values()) / len(models)
print(f"\nAkurasi test rata-rata: {avg:.2%}")
print(f"Total sektor: {len(models)}")
print("Model tersimpan di models/models_latest.pkl")
