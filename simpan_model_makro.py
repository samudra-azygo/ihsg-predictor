"""
simpan_model_makro.py
Training dengan fitur makro global:
S&P500, NASDAQ, Nikkei, Hang Seng, VIX, USD/IDR, EUR/IDR, US Bond yield
Target: akurasi 62-65%
"""
...
# 3. Makro global — buat fitur dari setiap sumber
MAKRO_FILES = {
    "sp500"   : "data/makro/SP500.csv",
    "nasdaq"  : "data/makro/NASDAQ.csv",
    "nikkei"  : "data/makro/NIKKEI.csv",
    "hangseng": "data/makro/HANGSENG.csv",
    "vix"     : "data/makro/VIX.csv",
    "usdidr"  : "data/makro/USDIDR.csv",
    "euridr"  : "data/makro/EURIDR.csv",
    "usbond"  : "data/makro/USBOND.csv",
}

dfs_makro = {}
for nama, path in MAKRO_FILES.items():
    ...
    # Fitur per sumber makro
    df[f"{nama}_pct"]      = df["close"].pct_change()
    df[f"{nama}_lag1"]     = df[f"{nama}_pct"].shift(1)
    df[f"{nama}_lag2"]     = df[f"{nama}_pct"].shift(2)
    df[f"{nama}_ma5"]      = df["close"].rolling(5).mean().pct_change()
    df[f"{nama}_trend"]    = (df["close"] > df["close"].rolling(20).mean()).astype(int)

    # Fitur khusus VIX — fear index
    if nama == "vix":
        df["vix_level"]    = df["close"]
        df["vix_tinggi"]   = (df["close"] > 25).astype(int)
        df["vix_panik"]    = (df["close"] > 35).astype(int)

    # Fitur khusus USD/IDR — kurs
    if nama == "usdidr":
        df["rupiah_lemah"] = (df["close"] > df["close"].rolling(20).mean()).astype(int)
        df["rupiah_volatil"]= df["close"].pct_change().rolling(5).std()

    # Fitur khusus US Bond — suku bunga
    if nama == "usbond":
        df["bond_naik"]    = (df["close"] > df["close"].shift(5)).astype(int)
        df["bond_level"]   = df["close"]

    cols = [c for c in df.columns if c != "close"]
    dfs_makro[nama] = df[cols]
    print(f"  OK {nama:12s} - {len(df)} hari - {len(cols)} fitur")

# Gabungkan semua makro
df_makro_gabungan = pd.concat(dfs_makro.values(), axis=1)
df_makro_gabungan.to_csv("data/makro/MAKRO_GABUNGAN.csv")
print(f"\nTotal fitur makro: {len(df_makro_gabungan.columns)}")

# Gabungkan semua eksterior
df_eksterior = df_kom.join(df_cuaca, how="outer").join(df_makro_gabungan, how="outer")
print(f"Total fitur eksterior: {len(df_eksterior.columns)}")
print(f"  Komoditas: {len(df_kom.columns)}")
print(f"  Cuaca    : {len(df_cuaca.columns)}")
print(f"  Makro    : {len(df_makro_gabungan.columns)}")
...
# ── Training ──────────────────────────────────────────────────
models = {}
print("\nTraining per sektor dengan makro global...")
total_acc = []
...
with open("models/models_makro.pkl", "wb") as f:
    pickle.dump(models, f)
...
print(f"\nPerbandingan:")
print(f"  Model cuaca (terbaik) : 59.01%")
print(f"  Model + makro global  : {avg:.2%}")
print(f"  Perubahan             : {avg-0.5901:+.2%}")
